...
#		OMP_PLACES=threads OMP_PROC_BIND=true OMP_NESTED=true PMEM_IS_PMEM_FORCE=1 LD_PRELOAD=/usr/lib/libjemalloc.so numactl $numaarg ./new-src/cceh_bench 2 ${workload[$i]} ${thread_num[$j]} 
		OMP_PLACES=threads OMP_PROC_BIND=true OMP_NESTED=true PMEM_IS_PMEM_FORCE=1 LD_PRELOAD=/usr/lib/libjemalloc.so numactl $numaarg ./build/test_pmem 64 ${workload[$i]} ${thread_num[$j]} #>> cuckoo_finger.txt
