...
inline void clflush(char* data, size_t len) {
  ...
  for (; ptr < data+len; ptr+=CACHE_LINE) {
    unsigned long etcs = ReadTSC() + (unsigned long)
(kWriteLatencyInNS*CPU_FREQ_MHZ/1000); asm volatile("clflush %0" : "+m"
(*(volatile char*)ptr)); while (ReadTSC() < etcs) CPUPause(); clflushCount++;
  }
  mfence();
}
*/
