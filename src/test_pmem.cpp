...
DEFINE_string(k, "fixed", "the type of stored keys");
...
DEFINE_int32(p, 20000000, "the number of operations(insertion/search/deletion) to execute");
DEFINE_string(op, "full", "which type of operation to execute");
...
struct range {
  ...
  int begin;
  int end;
  int length;/*if this is the variable lenght key, use this parameter to indicate the length of the key*/
  void* workload;
  uint64_t random_num;
};
...
void generate_16B(void *memory_region, int generate_num, int length, bool persist) {
  string_key* var_key;
  uint64_t* _key;
  ...
  for (int i = 0; i < generate_num; ++i) {
    var_key = reinterpret_cast<string_key *>(workload + i * (length + sizeof(string_key)));
  ...
  if (persist) {
    Allocator::Persist(memory_region, generate_num * (sizeof(string_key) + length));
  }
}

template<class T>
void Load(int kv_num, Finger_EH<T>* index, int length){
    std::cout << "First loading " << kv_num <<" keys" << std::endl;
    if(kv_num == 0) return;
...
template<class T>
void GeneralBench(range *rarray, Finger_EH<T> *index, int thread_num, int operation_num, std::string profile_name,
...
void* GenerateWorkload(int generate_num, int length){
    /*Since there are both positive search and negative search, it should generate 2 * generate_num workload*/
    void* workload;
    if(key_type == "fixed"){
...
void Run(){
    ...
    /* Load Phase, first generate workload, if the key is variable length key, we use 16B key to represent*/
    ...
    if(key_type == "variable"){
         ...
         memcpy(insert_workload, workload, (sizeof(string_key) + 16) * operation_num * 2);
    }else{  
    ...
    }else if(operation == "mixed"){/*mixed workload, the default setting is read 80%, insert 20%*/
