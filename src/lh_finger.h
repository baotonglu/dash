...
/*
void Linear::FindAnyway(Key_t key){
  auto key_hash = h(&key, sizeof(key));
  auto meta_hash = META_HASH(key_hash);
  auto y = BUCKET_INDEX(key_hash);
RETRY:
	...
	//auto x = IDX(key_hash, segmentSize*static_cast<uint64_t>(pow(2, N)));
	auto x = IDX(key_hash, N);
	if (x < next)
	{
		//x = IDX(key_hash, 2*segmentSize*static_cast<uint64_t>(pow(2, N)));
		x = IDX(key_hash, N+1);
	}
	...
	assert(dir_idx < directorySize);
	Table* target = dir->_[dir_idx] + offset;
  
	Bucket *target_bucket = target->bucket + y;
	Bucket *neighbor_bucket = target->bucket + ((y+1) & bucketMask);
	//uint32_t dir_index = SEG_IDX(static_cast<uint32_t>(pow(2, N)) + next);
	uint32_t dir_index;
	uint32_t _offset;
	SEG_IDX_OFFSET(static_cast<uint32_t>(pow(2, N)) + next, dir_index, _offset);

	for (int k = 0; k < dir_idx+1; ++k)
	{
		//printf("search in segment %d\n", k);
		for (int j = 0; j < segmentSize; ++j)
		{
			Table *curr_table = dir->_[k] + j;
			for (int i = 0; i < kNumBucket; ++i)
			{
				Bucket *curr_bucket = curr_table->bucket + i;
				auto ret = curr_bucket->check_and_get(meta_hash, key, false);
				if (ret != NONE)
				{
					printf("find in the original bucket %d, in table %d, with false\n", i, k*segmentSize + j);
					return;
				}
				...
				if (ret != NONE)
				{
					printf("find in the original bucket %d, in table %d, with true\n", i, k*segmentSize + j);
					return;
				}
			}
			...
			for (int i = 0; i < stashBucket; ++i)
			{
				overflowBucket *curr_bucket = curr_table->stash + i;
				auto ret = curr_bucket->check_and_get(meta_hash, key);
				if (ret != NONE)
				{
					printf("find in the stash bucket %d, in table %d\n", i, k*segmentSize + j);

					printf("the stash check is %d\n", target_bucket->test_stash_check());
					printf("the overflow bit is %d\n", target_bucket->test_overflow());
					printf("the image of target_bucket\n");
					int mask = target_bucket->finger_array[14];
					for (int j = 0; j < 4; ++j)
					{
						printf("the hash is %d, the member bit is %d, the alloc bit is %d, the stash bucket info is %lu, the real stash bucket info is %d\n", target_bucket->finger_array[15 + j], (target_bucket->overflowMember >> (j)) & 1, (target_bucket->finger_array[14] >> j) & 1, (target_bucket->finger_array[19] >> (j*2)) & stashMask, i);
					}
			...
			overflowBucket *prev_bucket = curr_table->stash;
			overflowBucket *next_bucket = curr_table->stash->next;
			while(next_bucket != NULL){
				auto ret = next_bucket->check_and_get(meta_hash, key);
				if (ret != NONE)
				{
					printf("find in the overflow bucket in table %d\n", k*segmentSize + j);
					return;
				}
				prev_bucket = next_bucket;
				next_bucket = next_bucket->next;
	...
	printf("No findings for the key\n");
}

void Table::PrintTableImage(Table* table, uint64_t base_level){
	for (int i = 0; i < kNumBucket; ++i)
	{
		Bucket *curr_bucket = table->bucket + i;
		printf("For Bucket %d:\n", i);

		int bitmap = GET_BITMAP(curr_bucket->bitmap);
		for (int j = 0; j < kNumPairPerBucket; ++j)
		{
			if (CHECK_BIT(bitmap, j))
			{
				printf("key %d: %zu; ", j, curr_bucket->_[j].key);
			}
		}

		printf("The Extra Fingerprint Info:\n");
		int mask = curr_bucket->finger_array[14];
		for (int j = 0; j < 4; ++j)
		{
			printf("the meta_hash is %d, the member bit is %d, the alloc bit is %d, the stash bucket info is %d\n", \
			 curr_bucket->finger_array[15 + j], (curr_bucket->overflowMember >> (j)) & 1, (curr_bucket->finger_array[14] >> j) & 1, \
			 (curr_bucket->finger_array[19] >> (j*2)) & low2Mask);
		}
		printf("the overflow count is %d\n", curr_bucket->overflowCount);
		printf("the stash_check is %d\n", curr_bucket->test_stash_check());
	}

	for (int i = 0; i < stashBucket; ++i)
	{
		printf("for Stash %d\n", i);
		overflowBucket *curr_bucket = table->stash + i;
		int bitmap = GET_BITMAP(curr_bucket->bitmap);
		for (int j = 0; j < kNumPairPerBucket; ++j)
		{
			if (CHECK_BIT(bitmap, j))
			{
				auto key_hash = h(&curr_bucket->_[j].key, sizeof(Key_t));
				auto index = BUCKET_INDEX(key_hash);
				printf("key %d, %zu, meta_hash is %d, the original bucket is %lu\n",j, \
					curr_bucket->_[j].key, curr_bucket->finger_array[j], index);
			}
		}
		printf("\n");
	}
	...
	int count = 0;
	overflowBucket *prev_bucket = table->stash;
	overflowBucket *next_bucket = table->stash->next;
	while(next_bucket != NULL){
		printf("for Overflow Bucket %d\n", count);		
		int bitmap = GET_BITMAP(next_bucket->bitmap);
		for (int i = 0; i < kNumPairPerBucket; ++i)
		{
			if (CHECK_BIT(bitmap, i))
			{
				auto key_hash = h(&next_bucket->_[i].key, sizeof(Key_t));
				auto index = BUCKET_INDEX(key_hash);
				printf("key %d, %zu, meta_hash is %d, the original bucket is %lu\n",i, \
					next_bucket->_[i].key, next_bucket->finger_array[i], index);
			}
		}
		prev_bucket = next_bucket;
		next_bucket = next_bucket->next;
