...
/*-----------------------------------------------Concurrent Insertion Test-----------------------------------------------------------------------*/
	...
	//System::profile("Insertion", [&](){
		...
		printf ("For %d threads, Insert Total time = %f seconds, the throughput is %f options/s\n", thread_num,
	         (double) (tv2.tv_usec - tv1.tv_usec) / 1000000 +
	         (double) (tv2.tv_sec - tv1.tv_sec), insert_num/duration);
	//});
	
	//printf("the collison check is %d\n", eh->count);
	LOG("Concurrent insertion end------------------------------------------------------------------");
	eh->getNumber();
	//eh->CheckDepthCount();
/*-----------------------------------------------Concurrent Get Test-----------------------------------------------------------------------*/
	...
	//System::profile("NP_search", [&](){
		...
		gettimeofday(&tv2, NULL);
	    duration = (double) (tv2.tv_usec - tv1.tv_usec) / 1000000 + (double) (tv2.tv_sec - tv1.tv_sec);
		printf ("For %d threads, Get Total time = %f seconds, the throughput is %f options/s\n", thread_num,
	         (double) (tv2.tv_usec - tv1.tv_usec) / 1000000 +
	         (double) (tv2.tv_sec - tv1.tv_sec), insert_num/duration);
	//});
	...
	printf ("For %d threads, Get Total time = %f seconds, the throughput is %f options/s\n", thread_num,
         (double) (tv2.tv_usec - tv1.tv_usec) / 1000000 +
         (double) (tv2.tv_sec - tv1.tv_sec), insert_num/duration);	
	LOG("Concurrent negative get end!---------------------------------------------------------------");
	return 0;
}
